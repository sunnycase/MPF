...
void PlatformProvider<PlatformId::D3D9>::PlayRenderCall(const PlayRenderCallArgs<PlatformId::D3D9>& args)
{
	...
	for (auto&& rc : args.StrokeRenderCalls)
	{
		...
		ThrowIfFailed(device->SetStreamSource(0, vb.Get(), 0, rc.VB.Stride));
		ThrowIfFailed(device->SetIndices(ib.Get()));
		...
		ThrowIfFailed(device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, rc.VB.Start, rc.VB.Start, rc.VB.Count, rc.IB.Start, rc.PrimitiveCount));
	}
}